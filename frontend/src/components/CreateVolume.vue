    ...
    <q-dialog v-model="layout">
        ...
        <q-card-section class="q-pt-none">
            <q-form>
                ...
                <div class="row">
                    <div class="col">
                        <q-select label="Pool" v-if="showPoolSelection" v-model="selectedPool" :options="poolOptions" />
                    </div>
                </div>
                <div class="row">
                    <div class="col">
                        <q-input label="Size" v-model="volumeSize" type="number" min="1"/>
                    </div>
                    <div class="col-md-auto">
                        <q-select v-model="volumeSizeUnit" :options="volumeSizeUnitOptions" />
                    </div>
                </div>
                <div class="row">
                    <div class="col">
                        <q-select label="Format" v-model="volumeFormat" :options="volumeFormatOptions" />
                    </div>
                </div>
            </q-form>
        </q-card-section>
...
export default {
    ...
    methods: {
        ...
        createVolume() {
            ...
            this.$api.post("/storage-pools/" + this.pooluuid + "/volume/" + this.volumeName, formData)
                .then(
                    this.layout = false,
                    this.$emit("volume-created"),
                )
                .catch(error => {
                    this.$refs.errorDialog.show("Error creating volume", [error.response.data])
                })
        },
    }
}
