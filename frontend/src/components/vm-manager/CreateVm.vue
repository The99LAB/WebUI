        ...
        <q-page padding>
          <q-tab-panel name="general" v-show="tab == 'general'">
            <q-input
              ...
              :rules="[
                (val) => (val && val.length > 0) || 'Name needs to be at least 1 character long',
              ]"
            />
  ...
  methods: {
    ...
    getMachineTypes() {
      this.$api
        ...
        .catch((error) => {
          this.$refs.errorDialog.show('Failed to get machine types', [error.response.data.detail])
        })
    },
    getOvmf() {
    ...
    createVm() {
      ...
      if (this.$refs.networkSource.getSelectedNetwork() == null) {
        this.$refs.errorDialog.show(
          'Please select a network'[
            ('Please select a network', "Create a network if you don't have one")
          ],
        )
        return
      }
      ...
      this.$api
        ...
        .catch((error) => {
          this.$refs.errorDialog.show('Error creating VM', [error.response.data.detail])
        })
    },
    prevTabPanel() {
      if (this.tab === 'memory') {
