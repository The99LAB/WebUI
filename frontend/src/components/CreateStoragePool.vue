    ...
    <q-dialog v-model="layout">
        ...
        <q-card-section class="q-pt-none">
            <q-form>
                ...
                <div class="row">
                    <div class="col">
                        <q-select label="Type" v-model="poolType" :options="poolTypeOptions" />
                    </div>
                </div>
                <div class="row">
                    <div class="col">
...
export default {
    ...
    methods: {
        ...
        createPool() {
            ...
            formData.append("name", this.poolName)
            formData.append("type", this.poolType)
            formData.append("path", this.poolPath)
            this.$api.post("/storage-pools", formData)
                .then(
                    this.layout = false,
                    this.$emit("storagepool-created"),
                )
                .catch(error => {
                    this.$refs.errorDialog.show("Error creating pool", [error.response.data])
                })
        },
    }
}
