  ...
  <q-dialog v-model="dialogShown" full-width full-height>
    ...
    >
      <q-header bordered elevated>
        <q-toolbar>
          <q-toolbar-title v-if="dialogMode.startsWith('new')">Create container</q-toolbar-title>
      ...
      <q-page-container>
        <q-page padding>
          <div class="row" v-if="dialogMode == 'new'">
            ...
            <div class="col q-ml-md">
              ...
              <div class="text-subtitle2">Maintainer: {{ dialogData.maintainer }}</div>
              <p>{{ dialogData.description }}</p>
            </div>
          </div>
          <q-separator spaced="lg" inset color="transparent" v-if="dialogMode == 'new'" />
          <q-form @submit="templateSubmit" ref="form" class="q-gutter-sm">
              ...
              <template v-slot:option="scope">
                <q-item v-bind="scope.itemProps">
                  <q-item-section>
                    ...
                    <q-item-label caption>driver: {{ scope.opt.driver }}</q-item-label>
                  </q-item-section>
                </q-item>
              </template>
            ...
            <div v-for="(command, index) in dialogData.config.command" :key="command.name">
              <q-input
              ...
              >
                ...
                >
                  <q-btn icon="mdi-delete" flat @click="removeField('command', index)" />
                  <q-tooltip :offset="[0, 0]">Remove field</q-tooltip>
                </template>
              </q-input>
            </div>
            <q-separator spaced="xl" v-if="dialogData.config.env.some((item) => item.editable)" />
            <p class="text-h6 q-mb-none" v-if="dialogData.config.env.some((item) => item.editable)">
              Environment
            </p>
            <div v-for="(item, index) in dialogData.config.env" :key="item.name">
              <q-input
                v-model="item.value"
                :type="item.type == 'int' ? 'number' : item.type == 'str' ? 'text' : 'text'"
                :label="item.name"
              ...
              >
                <template
                  ...
                  v-if="dialogMode == 'edit-custom' || dialogMode == 'new-custom'"
                >
                  <q-btn icon="mdi-delete" flat @click="removeField('env', index)" />
                  <q-tooltip :offset="[0, 0]">Remove field</q-tooltip>
                </template>
              </q-input>
            </div>
            ...
            <div v-for="(item, index) in dialogData.config.volumes" :key="item.name">
              <DirectoryList
                class="q-mb-lg"
                v-model="item.value"
              ...
              >
                ...
                <template v-slot:hint> Container Path: {{ item.bind }} </template>
                <template
                  v-slot:append
                  v-if="dialogMode == 'edit-custom' || dialogMode == 'new-custom'"
                >
                  <q-btn icon="mdi-delete" flat @click="removeField('volume', index)" />
                  <q-tooltip :offset="[0, 0]">Remove field</q-tooltip>
                </template>
              </DirectoryList>
            </div>
            <q-separator
              spaced="xl"
              v-if="networkSelected.driver == 'bridge' && dialogData.config.ports.length > 0"
            />
            <p
              class="text-h6 q-mb-none"
              v-if="networkSelected.driver == 'bridge' && dialogData.config.ports.length > 0"
            ...
            <div v-for="(item, index) in dialogData.config.ports" :key="item.name">
              <q-input
                v-model="item.value"
                type="number"
              ...
              >
                ...
                <template
                  ...
                  v-if="dialogMode == 'edit-custom' || dialogMode == 'new-custom'"
                >
                  <q-btn icon="mdi-delete" @click="removeField('port', index)" flat />
                  <q-tooltip :offset="[0, 0]">Remove field</q-tooltip>
                </template>
              </q-input>
            </div>
        ...
        <q-footer reveal bordered elevated>
          <q-toolbar>
            ...
            />
            <q-space />
            <q-btn flat label="Reset" @click="() => $refs.form.resetValidation()" />
        ...
        <q-card-section>
          <q-form class="q-gutter-md" @submit="customFieldDialogSubmit">
            ...
            <q-input
              ...
              type="text"
              label="Name"
              v-if="customFieldData.type == 'command' || customFieldData.type == 'environment'"
            />
            <q-input v-model="customFieldData.description" type="text" label="Description" />
            <q-input
            ...
            <q-select
              ...
              v-if="customFieldData.type == 'port'"
            />
            <p v-if="customFieldData.notes.length > 0" class="q-mb-none">Notes:</p>
            <p v-for="note in customFieldData.notes" :key="note" class="q-mt-none q-mb-none">
              {{ note }}
            </p>
  ...
  methods: {
      ...
      this.$api
        ...
        .then((response) => {
          ...
          if (this.dialogMode.startsWith('new')) {
            ...
            this.networkSelected = this.networkOptions.find((item) => item.name == 'host')
        ...
        .catch((error) => {
          ...
          this.$refs.errorDialog.show('Error getting docker networks', [errormsg])
        })
    },
    fetchTemplate() {
    ...
    templateSubmit() {
      ...
      data.config.network = {
        ...
        ip: this.networkFixedIp && this.networkSelected.custom_ip ? this.networkIp : null,
      }
      if (this.dialogMode == 'new') {
    ...
    customFieldDialogShow() {
      ...
      if (this.dialogData.config.command.length > 0) {
        this.customFieldData.typeOptions = this.customFieldData.typeOptions.filter(
          (item) => item != 'command',
        )
      }
    ...
    customFieldDialogChangeType(value) {
      ...
      if (value == 'command') {
        ...
        this.customFieldData.description = 'Custom Environment Variable Description'
        ...
        this.customFieldData.volumemode = this.customFieldData.volumeModeOptions[0]
      } else if (value == 'port') {
