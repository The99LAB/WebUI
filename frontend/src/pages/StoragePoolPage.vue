<template>
    <q-page padding>
          ...
          <q-tr :props="props">
            <q-td key="name" :props="props">
              <q-btn flat round :icon="props.row.expand ? 'mdi-menu-down' : 'mdi-menu-right'"
                @click="props.row.expand = !props.row.expand" no-caps :label=props.row.name
            ...
            <q-td key="allocation" :props="props" class="text-weight-regular text-body2">
              {{ props.row.allocation }}
            </q-td>
            <q-td key="available" :props="props" class="text-weight-regular text-body2">
              {{ props.row.available }}
            </q-td>
          </q-tr>
              ...
              <q-tab-panels v-model="props.row.tab">
                <q-tab-panel name="overview">
                  <div class="row q-ma-sm">
                    <q-btn class="q-mr-sm" color="primary" icon="mdi-delete" label="Delete" @click="deleteStoragePool(props.row.uuid)" />
                    <q-btn  class="q-mr-sm" color="primary" icon="mdi-play" label="Activate" v-if="props.row.state != 'active'" @click="activateStoragePool(props.row.uuid)" />
                    <q-btn  class="q-mr-sm" color="primary" icon="mdi-stop" label="Deactivate" v-if="props.row.state == 'active'" @click="deactivateStoragePool(props.row.uuid)"  />
                    <q-btn class="q-mr-sm" color="primary" icon="mdi-play" :label="props.row.autostart ? 'Enable autostart' : 'Disable autostart'" @click="toggleAutostartStoragePool(props.row.uuid)" />
                  </div>
                  <div class="row q-ma-sm">
                ...
                <q-tab-panel name="volumes">
                    <div class="row">
                        <q-space />
                        <q-btn class="q-ma-sm" color="primary" icon="mdi-delete" label="Remove" v-if="props.row.selectedVolume !== undefined && props.row.selectedVolume.length" @click="removeVolume(props.row.uuid, props.row.selectedVolume)"/>
                        <q-btn class="q-ma-sm" color="primary" icon="mdi-plus" label="Create volume" @click="createVolume(pooluuid = props.row.uuid)" />
                    </div>
                    <div class="q-pa-md">
                    <q-table
      ...
      <CreateVolume ref="createVolumeDialog" @volume-created="getStoragePools"></CreateVolume>
      <createStoragePool ref="createStoragePoolDialog" @storagepool-created="getStoragePools"></createStoragePool>
    </q-page>
  </template>
  ...
  const columns = [
    ...
    { label: 'Allocation', field: 'allocation', name: 'allocation', align: 'left' },
    ...
    methods: {
      getStoragePools(){
        ...
        for (let i = 0; i < array1.length; i++) {
          ...
          array1SpecialData.push({uuid: uuid, expand: expand, tab: tab, volumes: volumes})
        }
      ...
      removeVolume(pooluuid, volume){
        ...
        this.$api.delete("/storage-pools/" + pooluuid + "/volume/" + volumeName).then((response) => {
      ...
      activateStoragePool(pooluuid){
        this.$api.post("/storage-pools/" + pooluuid + "/start").then((response) => {
        }).catch((error) => {
          this.$refs.errorDialog.show(error.response.data)
        })
      },
      deactivateStoragePool(pooluuid){
        this.$api.post("/storage-pools/" + pooluuid + "/stop").then((response) => {
        }).catch((error) => {
          this.$refs.errorDialog.show(error.response.data)
        })
      },
      deleteStoragePool(pooluuid){
        this.$api.post("/storage-pools/" + pooluuid + "/delete").then((response) => {
        }).catch((error) => {
          this.$refs.errorDialog.show(error.response.data)
        })
      },
      toggleAutostartStoragePool(pooluuid){
        this.$api.post("/storage-pools/" + pooluuid + "/toggle-autostart").then((response) => {
        }).catch((error) => {
          this.$refs.errorDialog.show(error.response.data)
        })
      },
