<template>
  <q-page padding>
    ...
    <div class="row q-gutter-sm">
      ...
      <q-btn icon="mdi-delete" label="Remove from vm" color="primary" @click="removeHotplugUsb" />
    </div>
  ...
  methods: {
    addHotplugUsb() {
      if (this.$refs.vmList.getSelectedVm() == null) {
        this.$refs.errorDialog.show('Error adding USB device to VM', ['No VM selected'])
        return
      }
      if (
        ...
        this.$refs.errorDialog.show('Error adding USB device to VM', ['No USB device selected'])
        return
      }
      this.$api
        .post('/vm-manager/' + this.$refs.vmList.getSelectedVm()['uuid'] + '/edit-usbhotplug-add', {
          vendorid: this.$refs.usbDevicesList.getSelectedUsbVendorId(),
          productid: this.$refs.usbDevicesList.getSelectedUsbProductId(),
        })
        .catch((error) => {
          this.$refs.errorDialog.show('Error adding USB device to VM', [error.response.data.detail])
        })
    },
    removeHotplugUsb() {
      if (this.$refs.vmList.getSelectedVm() == null) {
        this.$refs.errorDialog.show('Error removing USB device from VM', ['No VM selected'])
        return
      }
      if (
        this.$refs.usbDevicesList.getSelectedUsbVendorId() == null ||
        ...
        this.$refs.errorDialog.show('Error removing USB device from VM', ['No USB device selected'])
        return
      }
      this.$api
        .post(
          '/vm-manager/' + this.$refs.vmList.getSelectedVm()['uuid'] + '/edit-usbhotplug-delete',
          {
            vendorid: this.$refs.usbDevicesList.getSelectedUsbVendorId(),
            productid: this.$refs.usbDevicesList.getSelectedUsbProductId(),
          },
