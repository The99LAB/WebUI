        ...
        >
            <template v-slot:top-right>
                <div class="q-gutter-sm">
                    ...
                    @click="renameDialogItem = JSON.parse(JSON.stringify(selected[0])); renameDialog = true"
                    >
                    ...
                    <q-btn
                        ...
                        @click="this.$refs.confirmDialog.show('Are you sure?', ['Do you really want to remove this?', 'This action is not reversable!'], removePath)"
                    >
                        <q-tooltip :offset="[0,5]">
                            Remove
                        </q-tooltip>
            ...
            <template v-slot:top-left>
                <div v-for="(item, index) in currentPath.split('/')" :key="index" >
                   <div v-if="index == 0" 
                    ...
                    <div v-else-if="item != ''" class="row items-center"
                        style="cursor: pointer"
                        @click="getPath(currentPath.split('/').slice(0, index + 1).join('/'))"
                    >
            ...
            <template v-slot:body="props" >
                <q-tr :props="props" @click="clickRow(props.row)" style="cursor: pointer" v-if="props.row.name != '..'">
                    <q-td>
            ...
            <template v-slot:bottom-row>
                <q-tr no-hover>
                    <q-td/>
                    <q-td colspan="100%">
                        <q-item-label>{{ paths.filter(path => path.type.startsWith('dir') && path.name != '..').length }} directories, {{ paths.filter(path => path.type.startsWith('file')).length }} files</q-item-label>
                    </q-td>
                </q-tr>
            </template>
...
export default {
    ...
    methods: {
        ...
        getPath(path) {
            ...
            this.$api.post("system/file-manager", { path: path })
            .then((response) => {
        ...
        removePath(){
            for(let i=0; i < this.selected.length; i++){
                ...
                this.$api.post("system/file-manager/remove", { path: path })
                .then((response) => {
        ...
        folderDialogSubmit(){
            ...
            this.$api.post("system/file-manager/create-folder", { path: path, name: name })
            .then((response) => {
                this.getPath(this.currentPath)
        ...
        renameDialogSubmit(){
            ...
            this.$api.post("system/file-manager/rename", { path: path, name: name })
            .then((response) => {
                this.getPath(this.currentPath)
